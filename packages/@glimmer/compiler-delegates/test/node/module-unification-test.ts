...
module("Module Unification Delegate", {
  beforeEach() {
    let projectPath = findup(
      "packages/@glimmer/compiler-delegates/test/node/fixtures/code-gen"
    );
...
test("can generate the serialized constants pool ", assert => {
  ...
  assert.equal(
    serializedPool,
    `const pool =JSON.parse(${JSON.stringify(JSON.stringify(pool))});`
  );
});
...
test("can generate symbol tables", assert => {
  symbolTables.set(
    { name: "default", module: "./src/ui/components/x/template.hbs" },
    {
      hasEval: 0,
      symbols: []
    } as any
